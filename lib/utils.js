 ...
 * @returns {string} - ISO string in UTC (for TIMESTAMPTZ storage)
 */
...
/**
 * @deprecated Use getUTCISOString() instead - this name was misleading
 */
export const getISTISOString = getUTCISOString;
 ...
 * Note: Returns UTC ISO string which is correct for TIMESTAMPTZ columns.
...
/**
 * Format date for email display in IST (long format with weekday)
 * Example: "Saturday, January 25, 2026"
 * Uses en-US locale for consistency with existing email templates.
 * @param {Date|string} date - Date to format
...
/**
 * Format time for email display in IST
 * Example: "10:30 AM"
 * Uses en-US locale for consistency with existing email templates.
 * @param {Date|string} date - Date/time to format
 * @returns {string} - Formatted time string in IST
 */
export function formatTimeForEmail(date) {
  if (!date) return "N/A";
  try {
...
/**
 * Get IST date string for a date N days from now (YYYY-MM-DD format)
 * Useful for due dates in billing: formatDueDateIST(7) gives due date 7 days from now
 * @param {number} daysFromNow - Number of days to add to today
 * @returns {string} - Date in YYYY-MM-DD format in IST
 */
export function formatDueDateIST(daysFromNow) {
  const d = addDaysIST(new Date(), daysFromNow);
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
}

/**
 * Convert an IST date and time string to UTC ISO string for database storage.
 * This is used when the user provides a date (YYYY-MM-DD) and time (HH:mm)
 * that are explicitly in IST (e.g., WhatsApp booking flow).
 * @param {Date|string} date - Date (will have its date part used)
 * @param {string} timeStr - Time in "HH:mm" format (in IST)
 * @returns {string} - UTC ISO string
 */
export function istDateTimeToUTC(date, timeStr) {
  const d = new Date(date);
  const year = d.getFullYear();
  const month = String(d.getMonth() + 1).padStart(2, "0");
  const day = String(d.getDate()).padStart(2, "0");
  // Construct an ISO string with the explicit IST offset (+05:30)
  const isoWithOffset = `${year}-${month}-${day}T${timeStr}:00+05:30`;
