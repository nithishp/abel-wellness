...
import { supabaseAdmin, TABLES, ROLES, APPOINTMENT_STATUS } from "@/lib/supabase.config";
import { sendTextMessage, sendButtonMessage, sendListMessage } from "../service";
import { updateConversation, updateContext, resetConversation, logMessage, linkConversationToUser } from "../conversation";
...
export async function handleBookingStep(conversation, phone, message, messageType) {
...
async function handleName(conversation, phone, message) {
  ...
  if (name.length < 2 || name.length > 100) {
    await sendTextMessage(phone, templates.invalidInput("Please enter a valid full name (2–100 characters)."));
    return;
  }
  ...
  await sendTextMessage(phone, `Thanks, *${name}*! 👍\n\n${templates.askEmail()}`);
...
async function handleReason(conversation, phone, message) {
  ...
  if (reason.length < 2) {
    await sendTextMessage(phone, templates.invalidInput("Please describe your reason for visiting."));
    return;
  }
...
async function handleTime(conversation, phone, message, messageType) {
  ...
  if (!selectedSlot) {
    await sendTextMessage(phone, templates.invalidInput("Please select a valid time slot from the list."));
    return;
  }
  ...
  await logMessage(conversation.id, phone, "outbound", "Sent booking confirmation");
}
...
async function handleConfirmation(conversation, phone, message, messageType) {
  ...
  if (isCancel) {
    await resetConversation(conversation.id);
    await sendTextMessage(phone, "Appointment booking cancelled. No worries!\n\n_Type \"menu\" to see other options._");
    return;
  }
  ...
  try {
    ...
    const nameParts = ctx.name.trim().split(/\s+/);
    ...
    await sendTextMessage(phone, templates.bookingSuccess({
      date: ctx.formattedDate,
    ...
    await logMessage(conversation.id, phone, "outbound", `Appointment created: ${newAppointment.id}`);
    await resetConversation(conversation.id);
  } catch (error) {
