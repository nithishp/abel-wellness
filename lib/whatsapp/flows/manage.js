...
export async function handleStatusStep(conversation, phone, message) {
  if (conversation.current_step === STATUS_STEPS.AWAITING_EMAIL) {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      await sendTextMessage(
        phone,
        templates.invalidInput("Please enter a valid email address."),
      );
      return;
    }
...
async function showAppointmentStatus(conversation, phone, user) {
  ...
  await logMessage(
    ...
    phone,
    "outbound",
    `Status check: ${appointments.length} appointments`,
  );
  await resetConversation(conversation.id);
}
  ...
  const sections = [
    {
      ...
      rows: appointments.map((apt, i) => ({
        ...
        description:
          `${apt.status.toUpperCase()} — ${apt.reason_for_visit || "General"}`.substring(
            0,
            72,
          ),
      })),
...
export async function handleCancelStep(
  ...
  if (step === CANCEL_STEPS.AWAITING_SELECTION) {
    ...
    if (!appointmentId) {
      await sendTextMessage(
        phone,
        templates.invalidInput("Please select an appointment from the list."),
      );
      return;
    }
    ...
    if (!isConfirm) {
      await sendTextMessage(
        phone,
        'Appointment cancellation aborted. Your appointment is still active.\n\n_Type "menu" for main menu._',
      );
      await resetConversation(conversation.id);
      return;
    }
    ...
    try {
      ...
      await logMessage(
        ...
        phone,
        "outbound",
        `Cancelled appointment: ${ctx.selectedAppointmentId}`,
      );
    } catch (error) {
      console.error("Cancel error:", error);
      await sendTextMessage(
        phone,
        'Sorry, we couldn\'t cancel the appointment. Please try again or contact us.\n\n_Type "menu" for main menu._',
      );
    }
...
export async function handleRescheduleStep(
  conversation,
  phone,
  message,
  messageType,
  ...
  if (step === RESCHEDULE_STEPS.AWAITING_SELECTION) {
    ...
    if (date < todayIST || date.getDay() === 0) {
      await sendTextMessage(
        phone,
        templates.invalidInput(
          "Date must be a future weekday (Monday–Saturday).",
        ),
      );
      return;
    }
    ...
    if (!selectedSlot) {
      await sendTextMessage(
        phone,
        templates.invalidInput("Please select a valid time slot."),
      );
      return;
    }
    ...
    if (!isConfirm) {
      await sendTextMessage(
        phone,
        'Reschedule cancelled. Your appointment remains as is.\n\n_Type "menu" for main menu._',
      );
      await resetConversation(conversation.id);
      return;
    }
    ...
    try {
      ...
      // Schedule new reminders for the rescheduled date
      if (conversation.user_id) {
        await scheduleAppointmentReminders(
          phone,
          conversation.user_id,
          ctx.selectedAppointmentId,
          new Date(newDateUTC),
          conversation.context?.name || "Patient",
        );
      }
      ...
      await logMessage(
        ...
        phone,
        "outbound",
        `Rescheduled appointment: ${ctx.selectedAppointmentId}`,
      );
    } catch (error) {
      console.error("Reschedule error:", error);
      await sendTextMessage(
        phone,
        'Sorry, we couldn\'t reschedule the appointment. Please try again or contact us.\n\n_Type "menu" for main menu._',
      );
    }
