...
export async function handleIncomingMessage(parsedMessage) {
  ...
  try {
    // Run markAsRead and getOrCreateConversation in parallel — they are independent
    const [, conversation] = await Promise.all([
      messageId ? markAsRead(messageId) : Promise.resolve(),
      getOrCreateConversation(phone),
    ]);

    // Log inbound message — fire-and-forget, never blocks the response
    logMessage(conversation.id, phone, "inbound", content, type, messageId);

    // Normalize text for command matching (moved early for opt-out checks)
    ...
    if (type === "unsupported") {
      await sendTextMessage(
        phone,
        'Sorry, I can only process text messages and button/list selections at the moment.\n\n_Type "menu" to see what I can help with._',
      );
      return;
    }

    // Handle opt-out ("STOP") and opt-in ("START")
    if (textLower === "stop") {
      await supabaseAdmin
      ...
      await sendTextMessage(
        phone,
        'You have been unsubscribed from notifications. You will no longer receive automated messages from us.\n\n_Reply "START" at any time to re-subscribe._',
      );
      await resetConversation(conversation.id);
      return;
    }

    if (textLower === "start" && conversation.opted_out) {
      await supabaseAdmin
        .from(WA_TABLES.CONVERSATIONS)
    ...
    // If user has opted out, only allow "start" command
    if (conversation.opted_out) {
      await sendTextMessage(
        phone,
        'You are currently unsubscribed from notifications.\n\n_Reply "START" to re-subscribe._',
      );
      return;
    }
    ...
    if (
      (textLower === "cancel" && conversation.current_step === "idle") ||
      textLower === "3"
    ) {
      if (conversation.current_step === "idle") {
...
async function showWelcome(conversation, phone) {
  ...
  // Show full menu with list message (supports all 5 options)
    ...
    [
      {
        ...
        rows: [
          ...
          },
          {
            ...
            title: "🔄 Reschedule",
            description: "Change your appointment date/time",
          },
        ],
      },
      {
        ...
        rows: [
          {
            id: ACTIONS.HELP,
            title: "ℹ️ Help",
            description: "Get help and contact info",
          },
        ],
      },
    ],
    "Abel Wellness",
    "Reply with a number or select from list",
  );
}
...
async function routeToFlow(conversation, phone, content, messageType) {
  ...
  switch (flow) {
    ...
    case FLOWS.RESCHEDULE:
      return await handleRescheduleStep(
        conversation,
        phone,
        content,
        messageType,
