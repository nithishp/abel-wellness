...
export const getPublishedBlogs = async (limit = 10) => {
  try {
    // Get database ID from client-side environment variables
    const databaseId = CLIENT_DATABASE_ID || process.env.NEXT_PUBLIC_DATABASE_ID;
    const blogsId = CLIENT_BLOGS_ID || process.env.NEXT_PUBLIC_BLOGS_ID || "blogs";

    // Check if database ID is configured
    if (!databaseId) {
      console.warn("Database ID is not configured for client-side. Returning empty result.");
      return { documents: [] };
    }
