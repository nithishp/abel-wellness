...
export async function generateInvoiceNumber() {
  ...
  // Use atomic database function to generate invoice number
  ...
  if (error) {
    ...
    // Fallback to old method if RPC fails (e.g., function doesn't exist)
...
export async function addPayment(paymentData) {
  ...
  try {
    // Use atomic payment function to prevent race conditions
    const { data: result, error: rpcError } = await supabase.rpc(
      "process_payment_atomic",
      {
        p_invoice_id: paymentData.invoice_id,
        p_patient_id: paymentData.patient_id,
        p_amount: parseFloat(paymentData.amount),
        p_payment_method: paymentData.payment_method,
        p_status: paymentData.status || PAYMENT_STATUS.COMPLETED,
        p_transaction_reference: paymentData.transaction_reference || null,
        p_payment_date: paymentData.payment_date || new Date().toISOString(),
        p_payment_gateway: paymentData.payment_gateway || null,
        p_gateway_order_id: paymentData.gateway_order_id || null,
        p_gateway_payment_id: paymentData.gateway_payment_id || null,
        p_gateway_signature: paymentData.gateway_signature || null,
        p_gateway_response: paymentData.gateway_response || null,
    ...
    if (rpcError) {
      console.error("Error in atomic payment:", rpcError);
