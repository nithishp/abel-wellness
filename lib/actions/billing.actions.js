...
export async function createInvoice(invoiceData) {
  ...
  const invoiceLevelDiscount = parseFloat(invoiceData.discount_amount) || 0;
  ...
  try {
    ...
    // Calculate due date (IST-aware)
    const invoiceDate = invoiceData.invoice_date || getTodayIST();
    const dueDate = invoiceData.due_date || formatDueDateIST(paymentDueDays);
    ...
    if (invoiceData.items && invoiceData.items.length > 0) {
      ...
      let { data: updatedInvoice, error: refreshError } = await supabase
        .from(BILLING_TABLES.INVOICES)
        .select("*")
        .eq("id", invoice.id)
        .single();
      ...
      // Re-apply invoice-level discount (the trigger overwrites it with item-level discounts)
      if (invoiceLevelDiscount > 0) {
        const correctedTotal =
          (parseFloat(updatedInvoice.subtotal) || 0) +
          (parseFloat(updatedInvoice.tax_amount) || 0) -
          invoiceLevelDiscount;
        const { data: finalInvoice, error: discountError } = await supabase
          .from(BILLING_TABLES.INVOICES)
          .update({
            discount_amount: invoiceLevelDiscount,
            discount_reason: invoiceData.discount_reason || null,
...
export async function updateInvoice(invoiceId, invoiceData) {
  ...
  try {
    ...
    if (invoiceData.items && invoiceData.items.length > 0) {
      ...
      // Add new items
      const itemsResult = await addInvoiceItems(invoiceId, invoiceData.items, {
        defaultTaxRate,
      });
      ...
      // After trigger recalculates totals from items, fix total_amount to include invoice-level discount
      if (invoiceData.discount_amount !== undefined) {
        const discount = parseFloat(invoiceData.discount_amount) || 0;
        const { data: triggerInvoice } = await supabase
          .from(BILLING_TABLES.INVOICES)
        ...
        if (triggerInvoice) {
          updateData.total_amount =
            (parseFloat(triggerInvoice.subtotal) || 0) +
            (parseFloat(triggerInvoice.tax_amount) || 0) -
            discount;
        }
      }
    }
    ...
    return { success: true, invoice: refreshedInvoice || data };
  } catch (error) {
...
export async function createRefund(refundData) {
  ...
  try {
    ...
    const { data: payment, error: paymentError } = await supabase
      .from(BILLING_TABLES.PAYMENTS)
      .select(
        "*, invoice:invoices(id, amount_paid, total_amount, amount_due, status)",
      )
    ...
    const invoiceTotal = parseFloat(payment.invoice.total_amount) || 0;
    const newAmountDue = invoiceTotal - Math.max(newAmountPaid, 0);
...
export async function createInvoiceFromAppointment(
  ...
  try {
    ...
    const result = await createInvoice({
      ...
      items,
      notes: `Consultation on ${new Date(appointment.date).toLocaleDateString(
        "en-IN",
