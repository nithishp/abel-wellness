...
export async function createQuickBill(params) {
  ...
  try {
    ...
    // Insert invoice items (exclude subtotal and taxable_amount as they don't exist in DB)
    const itemsToInsert = items.map((item, index) => {
      const { subtotal, taxable_amount, ...itemData } = item;
      return {
...
export async function createCombinedBill(params) {
  ...
  try {
    ...
    // Insert items (exclude subtotal and taxable_amount as they don't exist in DB)
